...
def split_multi_channels_COMPLEX(data, num_channels):
    in_shape = data.shape
    if len(in_shape) == 3:
        hop = in_shape[2] / num_channels #40
        tmp = np.zeros((in_shape[0], num_channels, in_shape[1], hop),dtype=np.complex_) #shape=(time/256, channels(2), 256, 40)
        for i in range(num_channels):
            tmp[:, i, :, :] = data[:, :, i * hop:(i + 1) * hop]
    else:
        print("ERROR: The input should be a 3D matrix but it seems to have dimensions ", in_shape)
