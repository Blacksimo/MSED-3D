...
def extract_mbe(_y, _sr, _nfft, _nb_mel):
    ...
    #print 'shape FFT: ', FFT.shape, FFT.max(), FFT.min(), FFT.mean()
...
for audio_filename in os.listdir(audio_folder):
    ...
    if is_mono:
        ...
        FFT_120 = FFT[0].T  # shape = (time, freq)
        FFT_240 = FFT[1].T  # shape = (time, freq)
        FFT_480 = FFT[2].T  # shape = (time, freq)
    else:
        # SONO 2 CANALI
        for ch in range(y.shape[0]):
            ...
            mbe_ch = mbe_ch.T
            FFT_ch_120 = FFT_ch[0].T  # shape = (time, freq)
            FFT_ch_240 = FFT_ch[1].T  # shape = (time, freq)
            FFT_ch_480 = FFT_ch[2].T  # shape = (time, freq)
            if mbe is None:
                mbe = mbe_ch
                FFT_120 = FFT_ch_120
                FFT_240 = FFT_ch_240
                ...
                FFT_120 = np.concatenate((FFT_120, FFT_ch_120), 1)
                FFT_240 = np.concatenate((FFT_120, FFT_ch_240), 1)
                FFT_480 = np.concatenate((FFT_120, FFT_ch_480), 1)
    print "FFT_120: ", FFT_120.shape
    print "FFT_240: ", FFT_240.shape
    print "FFT_480: ", FFT_480.shape
    print "mbe: ", mbe.shape
